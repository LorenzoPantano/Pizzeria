...
static void pizza_pronta(MYSQL *conn)
{
	...
	if (!setup_prepared_stmt(&prepared_stmt, "call pizzapronta(?)", conn))
	{
	...
	{
		printf("Pizza segnata come pronta correttamente...\n");
	}

	mysql_stmt_close(prepared_stmt);
}

static void visualizza_pizze_da_preparare(MYSQL *conn)
{
	...
	if (!setup_prepared_stmt(&prepared_stmt, "call visualizzaPizzeDaPreparare()", conn))
	{
		...
		if (num_fields > 0)
		{
			...
			for (i = 0; i < num_fields; ++i)
			{
				...
				case MYSQL_TYPE_VAR_STRING:
					...
					{
						rs_bind[i].buffer = (char *)&(aggiunta);
						rs_bind[i].buffer_length = sizeof(aggiunta);
						break;
					}
					break;
...
void run_as_pizzaiolo(MYSQL *conn)
{
	...
	printf("Switching to pizzaiolo role...\n");
	...
	while (true)
	{
		...
		printf("1) Visualizza pizze da preparare\n");
		printf("2) Segna pizza come pronta\n");
		printf("3) Quit\n");
